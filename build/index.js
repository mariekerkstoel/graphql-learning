...
// Since we're not using a real db, this file replaces it
let users = [{
  id: 1,
  name: "John Doe",
...
const resolvers = {
  Query: {
    user: (parents, {
      ...
      return _db__WEBPACK_IMPORTED_MODULE_0__["users"].find(user => parseInt(user.id) === parseInt(id));
    },
  ...
  Mutation: {
    createUser: (parent, {
      ...
      try {
        _db__WEBPACK_IMPORTED_MODULE_0__["users"].push({
          id,
          name,
          email,
          age
        });
        return _db__WEBPACK_IMPORTED_MODULE_0__["users"][_db__WEBPACK_IMPORTED_MODULE_0__["users"].length - 1];
      } catch (err) {
        throw err;
      }
    },
    updateUser: (parent, {
      ...
      try {
        const foundUser = _db__WEBPACK_IMPORTED_MODULE_0__["users"].find(user => parseInt(user.id) === parseInt(id));
        foundUser.name = name;
        foundUser.email = email, foundUser.age = age;
        return foundUser;
      } catch (err) {
        throw err;
      }
